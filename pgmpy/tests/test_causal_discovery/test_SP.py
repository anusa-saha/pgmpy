...
class TestSP:
    ...
    def test_returns_dag(self, chain_data):
        est = SP(ci_test="pearsonr")
        est.fit(chain_data)
    ...
    def test_adjacency_matrix(self, chain_data):
        est = SP(ci_test="pearsonr")
        est.fit(chain_data)
    ...
    def test_multiple_optimal_permutations(self):
        rng = np.random.default_rng(0)
        data = pd.DataFrame(rng.normal(size=(500, 3)), columns=["A", "B", "C"])
        est = SP(ci_test="pearsonr")
        est.fit(data)
        assert len(est.optimal_permutations_) > 1
